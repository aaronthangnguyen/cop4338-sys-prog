...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < thread_no; i++) {
    pthread_create(&deposit_thread_id[i], NULL, deposit,
                   (void *)100); // create a thread to deposit $100 100 times
    pthread_create(&withdraw_thread_id[i], NULL, withdraw,
                   (void *)100); // create a thread to withdraw $100 100 times
  }
  for (int i = 0; i < thread_no; i++) {
